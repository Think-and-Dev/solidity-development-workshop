...
describe("Ballot", () => {
  before(async () => {
    ...
    ballotContract.initialize('candidateA', candidateA_address)
  })

  describe("Deployment", () => {
    it("Should set the right owner", async () => {
      expect(await ballotContract.owner()).to.equal(await owner.getAddress())
    })

    it("Should set the right the candidates", async () => {
      await ballotContract.addCandidate('candidateB', candidateB_address)

      const candidateA = await ballotContract.getCandidate(1)
      const candidateB = await ballotContract.getCandidate(2)
    ...
    it("Should perform a vote", async () => {
      await ballotContract.connect(voterA).vote(1)
      await ballotContract.connect(voterB).vote(1, { value: 5000 })
      await ballotContract.connect(voterC).vote(2, { value: 1000 })
    ...
    it("Should return the balance of the contract", async () => {
      const balance = await ballotContract.getContractBalance()
      expect(balance).to.equal(6000)
    })

    it("Should only the owner determine the winner", async () => {
      try {
        await ballotContract.connect(voterA).closeBallot()
      } catch (error: any) {
    ...
    it("Should determine the winner", async () => {
      ...
      const winnerId = await ballotContract.winningCandidateId()
    ...
    it("Should not add a candidate if the ballot is closed", async () => {
      try {
        await ballotContract.addCandidate('candidateC', candidateA_address)
      } catch (error: any) {
        expect(error.message).to.contains('Ballot is closed.')
      }
    })

    it("Should not perform a vote if the ballot is closed", async () => {
      try {
        await ballotContract.connect(voterA).vote(1)
      } catch (error: any) {
        expect(error.message).to.contains('Ballot is closed.')
    ...
    it("Should not close the ballot if the ballot is closed", async () => {
      try {
        await ballotContract.closeBallot()
      } catch (error: any) {
        expect(error.message).to.contains('Ballot is closed.')
