...
contract Ballot is Initializable, OwnableUpgradeable {
    ...
    event BallotClosed(uint winningCandidateId);

    function initialize(string memory _name, address _candidateAddress) public initializer {
        ballotClosed = false;
        candidatesCount++;
    ...
    function addCandidate(string memory _name, address _candidateAddress) public onlyOwner {
        require(!ballotClosed, "Ballot is closed.");
        candidatesCount++;
        candidates[candidatesCount] = Candidate(candidatesCount, _name, 0, _candidateAddress);
    }
    ...
    function closeBallot() public onlyOwner {
        ...
        ballotClosed = true;
        emit BallotClosed(winningCandidateId);
